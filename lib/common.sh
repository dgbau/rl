...
detect_project() {
  ...
  if [[ -f "$dir/node_modules/.bin/openspec" ]] || \
     jq -e '.devDependencies["@fission-ai/openspec"] // .dependencies["@fission-ai/openspec"]' "$dir/package.json" &>/dev/null; then
    has_openspec=true
  fi
...
This repository uses the [Ralph Loop](ralph/README.md) for AI-assisted development with [Claude Code](https://docs.anthropic.com/en/docs/claude-code).
...
\`\`\`bash
./ralph/loop.sh interview    # Claude interviews you
./ralph/loop.sh bootstrap    # Create tickets from proposal
./ralph/loop.sh              # Build one ticket
./ralph/loop.sh --auto --pr  # Autonomous: bootstrap -> build all -> archive -> PR
...
- [\`ralph/README.md\`](ralph/README.md) -- complete Ralph Loop documentation
EOF
}
